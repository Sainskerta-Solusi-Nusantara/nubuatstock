...
/* ---------- Free Float (status pemenuhan BEI) ---------- */
...
function FreeFloatTab({ ff }: { ff: FfDashData }) {
  const [q, setQ] = React.useState("");
  ...
  if (ff.rows.length === 0) {
    return <p className="rounded-lg border border-border bg-card p-6 text-center text-sm text-muted-foreground">Data Free Float belum tersedia.</p>;
  }

  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Status pemenuhan <strong>Free Float</strong> (saham beredar publik) sesuai ketentuan BEI · posisi {fmtDateId(ff.snapshotDate) || "—"}.
      </p>
...
/* ---------- Shell ---------- */
const TABS = ["Ringkasan Saham", "Per Investor", "Konglo Stocks", "Metrik", "Perubahan Data", "Klasifikasi", "Free Float"] as const;

export function KlinikDashboard({ data, ff }: { data: DashData; ff: FfDashData }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {tab === "Free Float" && <FreeFloatTab ff={ff} />}
